...
const result={totalCount:1,stockInTotal:300,consumptionTotal:50,resetTotal:10,byType:[{fuelType:'gas_oil',quantity:50}],byTank:[{tankId:'t1',tankName:'الخزان الرئيسي',fuelType:'gas_oil',stockIn:300,consumption:50}],byVehicle:[{vehicleId:'v1',vehicleName:'كابسة',dbNumber:'DB-1',quantity:50}],rows:[{id:'m1',tankId:'t1',vehicleId:'v1',movementType:'vehicle_fill',quantity:-50,quantityBefore:300,quantityAfter:250,nextRefillDate:'2026-09-20',notes:'تعبئة',actorId:'u1',actorName:'مسؤول الكراج',createdAt:'2026-09-08T09:00:00Z',tankName:'الخزان الرئيسي',fuelType:'gas_oil',vehicleName:'كابسة',dbNumber:'DB-1',areaName:'الوليد',parentSector:'zaafaraniya'}],from:'2026-09-01',to:'2026-09-08'}
...
describe('مركز تقارير الكراج',()=>{
  ...
  it('يعرض الملخصات والتجميعات وسجل الحركات',()=>{render(<MemoryRouter><GarageReportsPage/></MemoryRouter>);expect(screen.getByText('تقارير الكراج المركزي')).toBeInTheDocument();expect(screen.getAllByText('استهلاك الآليات')).toHaveLength(2);expect(screen.getAllByText('الخزان الرئيسي').length).toBeGreaterThan(0);expect(screen.getByText('2026-09-20')).toBeInTheDocument();expect(screen.getAllByText('٥٠').length).toBeGreaterThan(0)})
