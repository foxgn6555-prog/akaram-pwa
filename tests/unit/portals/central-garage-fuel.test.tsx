...
describe('واجهات وقود الكراج',()=>{
  ...
  it('يبقي موعد التعبئة إلزامياً للمواد الأخرى',()=>{h.tank.fuelType='hydraulic';show(<FuelTypePage fuelType="hydraulic"/>);fireEvent.click(screen.getByTestId('fill-t1'));fireEvent.click(screen.getByTestId(`select-fill-vehicle-${h.vehicle.id}`));fireEvent.change(screen.getByTestId('fill-quantity'),{target:{value:'10'}});fireEvent.click(screen.getByTestId('fill-submit'));expect(screen.getByText(/موعد التعبئة التالية مطلوب/)).toBeInTheDocument();const fillDate=new Date(Date.now()+3*86400000).toISOString().slice(0,10);fireEvent.change(screen.getByTestId('fill-next-date'),{target:{value:fillDate}});fireEvent.click(screen.getByTestId('fill-submit'));expect(h.fill).toHaveBeenCalledWith(expect.objectContaining({nextRefillDate:fillDate}),expect.any(Object))})
