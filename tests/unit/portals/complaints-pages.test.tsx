...
const item = { id:'i1',complaintId:'c1',sequenceNo:1,referenceNo:'CMP-1',complaintStatus:'archived',sector:'karrada',
  ...
  managerNotes:null,reviewerNotes:null,receivedAt:'2026-09-03T08:00:00Z',inboxMessageId:'m1' }
const otherSectorItem = { ...item, id:'i2', complaintId:'c2', referenceNo:'CMP-Z', sector:'zaafaraniya', ticketName:'بريد الزعفرانية', receivedAt:'2026-09-04T08:00:00Z', inboxMessageId:'m2' }
vi.mock('@features/complaints',()=>({
  ...
  useComplaintItems:()=>({data:[item,otherSectorItem],isLoading:false}),useComplaintItemMedia:()=>({data:[]}),useComplaintItemsMedia:()=>({data:[]}),useComplaintManagers:()=>({data:[{userId:'mgr1',fullName:'مسؤول أول'}]}),
  ...
  useComplaintReportDownload:()=>({mutate}),useArchiveComplaintReport:()=>({mutate}),useComplaintContacts:()=>({data:[]}),useComplaintSettings:()=>({data:[]}),
...
describe('صفحات بوابة الشكاوى المكتملة',()=>{
  ...
  it('تعرض محرر القوالب وإنشاء التقرير',()=>{view(<TemplatesPage/>);expect(screen.getByText('صمّم، أنشئ المسودة، ثم راجع وأرسل')).toBeInTheDocument();expect(screen.getByText('إنشاء مسودة التقرير اليومي')).toBeInTheDocument()})
  it('تفصل البيانات بالقاطع ثم مجلد اليوم ولا تخلط مواقع الزعفرانية',()=>{view(<DataPage/>);expect(screen.getByText('بيانات الشكاوى')).toBeInTheDocument();expect(screen.queryByText('CMP-1/1')).not.toBeInTheDocument();fireEvent.click(screen.getByRole('button',{name:/قاطع الكرادة/}));expect(screen.queryByText('بريد الزعفرانية')).not.toBeInTheDocument();fireEvent.click(screen.getByRole('button',{name:/الخميس/}));expect(screen.getByText('CMP-1/1')).toBeInTheDocument();expect(screen.queryByText(/CMP-Z/)).not.toBeInTheDocument()})
