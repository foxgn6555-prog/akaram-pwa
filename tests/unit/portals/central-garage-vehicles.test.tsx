...
const h = vi.hoisted(() => ({
  ...
  restoreMutate: vi.fn(),
  vehicleFilter: vi.fn(),
  vehicleIdParam: vi.fn(),
  vehicleIdParam2: vi.fn(),
  recordDeparture: vi.fn(),
  recordReturn: vi.fn(),
  areas: [
    { id: 1, name: 'أرخيته', parentSector: 'karrada', sort: 1 },
    { id: 2, name: 'الرياض', parentSector: 'karrada', sort: 2 },
    { id: 3, name: 'الواثق', parentSector: 'karrada', sort: 3 },
    { id: 4, name: 'الجادرية', parentSector: 'karrada', sort: 4 },
    { id: 5, name: 'السندباد', parentSector: 'zaafaraniya', sort: 5 },
    { id: 6, name: 'الزعفرانية', parentSector: 'zaafaraniya', sort: 6 },
    { id: 7, name: 'ديالى', parentSector: 'zaafaraniya', sort: 7 },
    { id: 8, name: 'الوليد', parentSector: 'zaafaraniya', sort: 8 },
  ],
  vehicle: {
    id: 'v1',
    vehicleName: 'كابسة نفايات',
    dbNumber: 'DB-100',
    plateNumber: 'بغداد 123',
    chassisNumber: 'CH-100',
    vehicleCategory: 'compactor_large',
    ownershipType: 'owned',
    lessorName: null,
  ...
  departure: {
    ...
    shift: 'morning',
    sectorId: 1,
    departedAt: '2026-09-08T07:30:00Z',
    arrivedAt: '2026-09-08T08:00:00Z',
    siteDepartedAt: '2026-09-08T15:00:00Z',
    returnedAt: null,
    recipientManagerId: 'm1',
    recipientManagerName: 'مسؤول أرخيته',
    arrivalNotes: null,
    ...
    data: [
      {
        ...
        tankId: 't1',
        vehicleId: 'v1',
        movementType: 'vehicle_fill',
        quantity: -40,
        quantityBefore: 200,
        quantityAfter: 160,
        fuelType: 'hydraulic',
        unit: 'gallon',
        tankName: 'خزان الهيدروليك',
        nextRefillDate: '2026-09-20',
        notes: null,
        actorId: 'u1',
...
beforeEach(() => {
  ...
  h.vehicleFilter.mockClear()
  h.vehicleIdParam.mockClear()
  h.recordDeparture.mockReset()
  h.recordReturn.mockReset()
  h.departures = []
...
const renderPage = (node: React.ReactNode, path = '/') =>
  render(
...
describe('قاعدة بيانات آليات الكراج', () => {
  it('يعرض الآلية كتذكرة ملونة بالمعلومات الأساسية ورابط التفاصيل', () => {
    ...
    expect(screen.getByTestId('vehicle-card-v1')).toBeInTheDocument()
    expect(screen.getByText('كابسة نفايات')).toBeInTheDocument()
    expect(screen.getByText('علي حسن')).toBeInTheDocument()
    expect(screen.getByText(/قاطع الكرادة/)).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /كابسة نفايات/ })).toHaveAttribute(
      'href',
      '/central-garage/vehicles-database/v1',
    )
  })
  ...
  it('يدعم تصنيف الآلية والملكية المؤجرة ويطلب اسم الجهة المؤجرة', () => {
    ...
    fireEvent.change(screen.getByTestId('vehicle-category'), { target: { value: 'tanker' } })
    fireEvent.change(screen.getByTestId('vehicle-ownership'), { target: { value: 'rented' } })
    expect(screen.getByTestId('vehicle-lessor')).toBeInTheDocument()
    expect(screen.getByTestId('vehicle-contract')).toBeInTheDocument()
  })

  it('يوفر البحث وفلاتر المنطقة والشفت دون عرض قائمة فوضوية', async () => {
    renderPage(<VehiclesDatabasePage />)
    fireEvent.change(screen.getByTestId('vehicle-search'), { target: { value: 'DB-100' } })
    fireEvent.change(screen.getByTestId('vehicle-area-filter'), { target: { value: '1' } })
    fireEvent.change(screen.getByTestId('vehicle-shift-filter'), { target: { value: 'night' } })
    await waitFor(() =>
      expect(h.vehicleFilter).toHaveBeenLastCalledWith(
        expect.objectContaining({ search: 'DB-100', sectorId: 1, shift: 'night', pageSize: 24 }),
      ),
    )
  })
  ...
  it('يتحقق من كل حقول إضافة الآلية ثم يرسل الصورة والقاطع والشفت', () => {
    ...
    fireEvent.change(screen.getByTestId('vehicle-image'), { target: { files: [file] } })
    fireEvent.click(screen.getByTestId('vehicle-submit'))
    expect(h.createMutate).toHaveBeenCalledWith(
      expect.objectContaining({
        vehicleName: 'كابسة جديدة',
        dbNumber: 'DB-200',
        shift: 'night',
        sectorId: 8,
        image: file,
...
describe('انطلاقية السائقين وتفاصيل الآلية', () => {
  ...
  it('بدون انطلاقات: يعرض «لم تسجل انطلاقاً» وزر تسجيل انطلاق من الكراج', () => {
    renderPage(<DriversDispatchPage />)
    expect(screen.getByTestId('departure-state-pending')).toBeInTheDocument()
    expect(screen.getByText('لم تنطلق بعد')).toBeInTheDocument()
    ...
    expect(h.recordDeparture).toHaveBeenCalledWith(
  ...
  it('انطلاقة مفتوحة: يظهر «في الميدان» وزر تسجيل عودة إلى الكراج', () => {
    ...
    renderPage(<DriversDispatchPage />)
    expect(screen.getByTestId('departure-state-field')).toBeInTheDocument()
    expect(screen.getByTestId('dispatch-row-v1')).toHaveTextContent('في الطريق إلى الكراج')
  ...
  it('انطلاقة مغلقة اليوم: يعرض «عادت إلى الكراج» مع زر انطلاق جديد', () => {
    h.departures = [{ ...h.departure, returnedAt: '2026-09-08T15:30:00Z' }]
    renderPage(<DriversDispatchPage />)
    expect(screen.getByTestId('departure-state-returned')).toBeInTheDocument()
    expect(screen.getByTestId('depart-v1')).toBeInTheDocument()
  })
  ...
  it('يعرض الصورة وشارة DB في بطاقة الآلية', () => {
    renderPage(<VehiclesDatabasePage />)
    expect(screen.getByTestId('vehicle-card-v1')).toHaveTextContent('DB-100')
    expect(screen.getByTestId('vehicle-card-v1').querySelector('img')).not.toBeNull()
  })

  it('يمرر بارامتر المسار vehicleId إلى استعلام التفاصيل (عقد المسار ↔ الصفحة)', () => {
    renderDetail()
    expect(h.vehicleIdParam).toHaveBeenCalledWith('v1')
  ...
  it('تعرض صفحة التفاصيل والسجل والموعد التالي للتعبئة', () => {
    ...
    expect(screen.getByTestId('vehicle-detail-page')).toHaveTextContent('CH-100')
    expect(screen.getByTestId('vehicle-detail-page')).toHaveTextContent('سجل الانطلاقية')
    expect(screen.getByTestId('vehicle-detail-page')).toHaveTextContent('2026-09-20')
    expect(screen.getByTestId('vehicle-detail-page')).toHaveTextContent('غالون')
    expect(screen.getByTestId('vehicle-detail-page')).toHaveTextContent('خزان الهيدروليك')
  ...
  it('يعدل بيانات الآلية الأساسية دون تغيير الإسناد التاريخي', () => {
    ...
    fireEvent.change(screen.getByTestId('edit-vehicle-db'), { target: { value: 'DB-101' } })
    fireEvent.click(screen.getByTestId('edit-vehicle-submit'))
    expect(h.updateMutate).toHaveBeenCalledWith(
      expect.objectContaining({
        id: 'v1',
        vehicleName: 'كابسة محدثة',
        dbNumber: 'DB-101',
        plateNumber: 'بغداد 123',
        chassisNumber: 'CH-100',
      }),
  ...
  it('لا يسمح بالأرشفة بلا سبب كاف ويرسل السبب عند اكتماله', () => {
    ...
    fireEvent.change(screen.getByTestId('archive-reason'), {
      target: { value: 'خروج الآلية من الخدمة' },
    })
    expect(screen.getByTestId('archive-submit')).toBeEnabled()
    fireEvent.click(screen.getByTestId('archive-submit'))
    expect(h.archiveMutate).toHaveBeenCalledWith(
      { vehicleId: 'v1', reason: 'خروج الآلية من الخدمة' },
      expect.objectContaining({ onSuccess: expect.any(Function) }),
    )
  })
})
