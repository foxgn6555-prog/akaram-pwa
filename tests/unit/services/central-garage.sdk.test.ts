...
const h = vi.hoisted(() => {
  ...
  function chain() {
    ...
    for (const method of ['select', 'eq', 'is', 'order', 'limit', 'single', 'returns'])
...
function imageFile(type = 'image/jpeg', valid = true): File {
  const raw = valid
    ? type === 'image/png'
      ? Uint8Array.from([0x89, 0x50, 0x4e, 0x47, 0, 0, 0, 0])
...
describe('SDK الكراج — الآليات والانطلاقية', () => {
  it('يبحث ويفلتر الآليات بصفحات ويوقع روابط الصور', async () => {
    ...
    const result = await centralGarage.vehicles({
      search: ' علي ',
      sectorId: 8,
      shift: 'night',
      page: 2,
    ...
    expect(h.rpc).toHaveBeenCalledWith('garage_search_vehicles', {
      p_search: 'علي',
      p_sector_id: 8,
      p_shift: 'night',
      p_limit: 24,
  ...
  it('يجلب تفاصيل آلية واحدة مع المنطقة ورابط صورة موقّع', async () => {
    h.state.result = {
      data: {
        ...
        driver_name: 'علي',
        sector_id: 1,
        sectors: { name: 'أرخيته', parent_sector: 'karrada' },
        created_at: 'now',
        updated_at: 'now',
        archived_at: null,
      },
  ...
  it('يرفع صورة الآلية ثم ينشئها عبر RPC وينظف الصورة عند فشل التسجيل', async () => {
    ...
    expect(h.buckets[0]?.upload).toHaveBeenCalledWith(
      expect.stringMatching(/^garage-user\/[0-9a-f-]+\.jpg$/),
      input.image,
  ...
  it('يعدل حقول الآلية ويرفع الصورة الاختيارية عبر RPC', async () => {
    ...
    h.state.result = {
      data: {
        id: 'v1',
        vehicle_name: 'كابسة معدلة',
        db_number: 'DB-2',
        plate_number: 'P2',
        chassis_number: 'CH-2',
        image_path: 'garage-user/new.jpg',
        shift: 'morning',
        driver_name: 'علي',
        sector_id: 1,
        created_at: 'now',
    ...
    const result = await centralGarage.updateVehicle('v1', {
      vehicleName: 'كابسة معدلة',
      dbNumber: 'DB-2',
      plateNumber: 'P2',
      chassisNumber: 'CH-2',
      image,
    ...
    expect(h.rpc).toHaveBeenCalledWith(
      'garage_update_vehicle',
      expect.objectContaining({
        p_vehicle_id: 'v1',
        p_db_number: 'DB-2',
        p_image_path: expect.stringMatching(/^garage-user\//),
      }),
    )
    expect(result.vehicleName).toBe('كابسة معدلة')
  })
  ...
  it('يغير السائق والشفت والموقع عبر RPC تاريخي ويؤرشف ويستعيد بسبب', async () => {
    ...
    expect(h.rpc).toHaveBeenLastCalledWith('garage_archive_vehicle', {
      p_vehicle_id: 'v1',
      p_reason: 'انتهاء الخدمة',
    })
    h.state.result = {
      data: {
        id: 'v1',
    ...
    expect(h.rpc).toHaveBeenLastCalledWith('garage_restore_vehicle', {
      p_vehicle_id: 'v1',
      p_reason: 'عودة للخدمة',
    })
  })
})
...
describe('SDK الكراج — الخزانات والتعبئة والموافقات', () => {
  const movement = {
    ...
    tank_id: 't1',
    vehicle_id: 'v1',
    movement_type: 'vehicle_fill',
    quantity: -40,
    quantity_before: 200,
    quantity_after: 160,
    next_refill_date: '2026-09-20',
    notes: null,
    actor_id: 'u1',
    created_at: '2026-09-08T10:00:00Z',
  }
  ...
  it('يدعم الخزانات الأربعة وإضافة الكمية عبر RPC', async () => {
    ...
    expect(h.rpc).toHaveBeenCalledWith('garage_add_tank', {
      p_fuel_type: 'c_oil',
      p_tank_name: 'C1',
      p_unit: 'liter',
      p_capacity: 500,
      p_initial_quantity: 100,
      p_low_stock_threshold: 20,
    })
    ...
    expect(h.rpc).toHaveBeenLastCalledWith('garage_add_tank_stock', {
      p_tank_id: 't1',
      p_quantity: 50,
      p_notes: 'وجبة',
    })
  ...
  it('يجلب حركة الآلية مع وحدة الخزان ونوع المادة لعرض السجل بدقة', async () => {
    h.state.result = {
      data: [
        {
          ...movement,
          garage_tanks: { fuel_type: 'hydraulic', unit: 'gallon', tank_name: 'خزان الهيدروليك' },
        },
      ],
      error: null,
    }
  ...
  it('يرسل موعد التعبئة التالي ولا يرسل وقت العملية من العميل', async () => {
    ...
    expect(h.rpc).toHaveBeenCalledWith('garage_fill_vehicle', {
      p_tank_id: 't1',
      p_vehicle_id: 'v1',
      p_quantity: 40,
      p_next_refill_date: '2026-09-20',
      p_notes: 'تعبئة',
    })
    const payload =
      (h.rpc.mock.calls as unknown as Array<[string, Record<string, unknown>]>)[0]?.[1] ?? {}
    expect(
      Object.keys(payload).some((key) => key.includes('time') || key.includes('created')),
    ).toBe(false)
  ...
  it('يرسل طلب التصفير ثم القرار حصراً عبر RPC', async () => {
    ...
    expect(h.rpc).toHaveBeenCalledWith('garage_request_tank_zero', {
      p_tank_id: 't1',
      p_reason: 'مطابقة فعلية',
    })
    h.state.result = {
      data: {
        id: 'r1',
    ...
    expect(h.rpc).toHaveBeenLastCalledWith('garage_decide_tank_zero', {
      p_request_id: 'r1',
      p_approved: true,
      p_note: 'موافق',
    })
  ...
  it('يجلب تقرير الحركات بصفحات وفلاتر ويسترجع كامل الصفوف للتصدير', async () => {
    ...
    expect(h.rpc).toHaveBeenCalledWith('garage_consumption_report', {
      p_from: '2026-09-01',
      p_to: '2026-09-08',
      p_sector_id: 8,
      p_fuel_type: 'gas_oil',
      ...
      p_vehicle_id: 'v1',
      p_vehicle_ids: null,
      p_movement_type: 'vehicle_fill',
      p_limit: 50,
      p_offset: 50,
    })
    await centralGarage.report({ vehicleIds: ['v1', 'v2'], pageSize: 25 })
    expect(h.rpc).toHaveBeenLastCalledWith(
      'garage_consumption_report',
      expect.objectContaining({ p_vehicle_id: null, p_vehicle_ids: ['v1', 'v2'], p_limit: 25 }),
    )
    h.rpc
      .mockResolvedValueOnce({
        data: {
          ...base,
          totalCount: 101,
          rows: Array.from({ length: 100 }, (_, i) => ({ id: `m${i}` })),
        },
        error: null,
      })
      .mockResolvedValueOnce({
        data: { ...base, totalCount: 101, rows: [{ id: 'm100' }] },
        error: null,
      })
    const all = await centralGarage.reportAll({ fuelType: 'gas_oil' })
    expect(all.rows).toHaveLength(101)
  })
  ...
  it('يجلب ملخص التقارير بالفترة المحددة', async () => {
    h.state.result = {
      data: {
        vehiclesTotal: 8,
        driversTotal: 7,
        vehiclesByShift: { morning: 4 },
        vehiclesByArea: [],
        tankStock: [],
        consumptionByType: { gas_oil: 500 },
        pendingZeroRequests: 1,
        from: '2026-09-01',
        to: '2026-09-08',
      },
      error: null,
    }
    const summary = await centralGarage.dashboard({
      from: '2026-09-01',
      to: '2026-09-08',
      sectorId: 8,
      fuelType: 'gas_oil',
    })
    expect(h.rpc).toHaveBeenCalledWith('garage_dashboard_summary', {
      p_from: '2026-09-01',
      p_to: '2026-09-08',
      p_sector_id: 8,
      p_fuel_type: 'gas_oil',
      ...
      data: [
        {
          ...
          shift: 'morning',
          sector_id: 1,
          departed_at: '2026-09-08T07:30:00Z',
          returned_at: null,
          notes: null,
          vehicle_name: 'كابسة',
          db_number: 'DB-1',
  ...
  it('يجلب فهرس مجلدات الأيام ويحمل محتوى يوم واحد', async () => {
    h.state.result = {
      data: [
        {
          trip_day: '2026-09-09',
    ...
    const days = await centralGarage.departureDays()
    expect(h.rpc).toHaveBeenCalledWith('garage_departure_days', { p_limit: 60, p_offset: 0 })
    expect(days[0]).toMatchObject({ tripDay: '2026-09-09', totalCount: 4, openCount: 1 })
    h.state.result = { data: [], error: null }
    await centralGarage.departuresForDay('2026-09-09')
    expect(h.rpc).toHaveBeenLastCalledWith('garage_departures_for_day', { p_day: '2026-09-09' })
  })

  it('يجلب مسؤولي الاستلام المتوافقين عبر SDK', async () => {
    h.state.result = {
      data: [{ user_id: 'm1', manager_name: 'مسؤول الكرادة', shift: 'morning', sectors: [1, 2] }],
      error: null,
    }
    const rows = await centralGarage.dispatchRecipients('v1')
    expect(h.rpc).toHaveBeenCalledWith('garage_dispatch_recipients', { p_vehicle_id: 'v1' })
  ...
  it('يسجل انطلاق السائق وعودته عبر RPC حصراً (وقت الخادم)', async () => {
    ...
    expect(h.rpc).toHaveBeenCalledWith('garage_record_departure', {
      p_vehicle_id: 'v1',
      p_notes: 'خروج للوردية',
    })
    h.state.result = {
      data: {
        ...(h.state.result.data as Record<string, unknown>),
        returned_at: '2026-09-08T15:30:00Z',
      },
      error: null,
    }
