...
const report=readFileSync('supabase/functions/complaint-generate-report/index.ts','utf8')+readFileSync('src/lib/pptx/complaintPptx.ts','utf8')
describe('عقد صور المعالجة المتعددة المرتبة',()=>{
 ...
 it('يدعم WebP في ملف PowerPoint دون شريحتي المؤشرات والفاصل',()=>{expect(report).toContain('ContentType="image/webp"');expect(report).toContain("mime==='image/webp'?'webp'");expect(report).not.toContain('المؤشرات التنفيذية للتقرير');expect(report).not.toContain('مجموعة البريد والمسؤول')})
})
