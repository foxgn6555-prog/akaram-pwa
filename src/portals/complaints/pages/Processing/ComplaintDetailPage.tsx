  ...
  return <section className="min-h-full space-y-5 pb-8" dir="rtl">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_minmax(300px,.7fr)]">
      <main className="space-y-5">
        ...
        {canReview && <article className="rounded-3xl border border-emerald-200 bg-emerald-50 p-5 shadow-sm sm:p-6"><div className="flex items-center gap-3"><ShieldCheck className="text-emerald-700" /><div><h2 className="font-black text-emerald-950">قرار التدقيق النهائي</h2><p className="text-xs text-emerald-800">تأكد من تطابق الموقع وصور قبل وبعد قبل اتخاذ القرار.</p></div></div><textarea aria-label="ملاحظة التدقيق" value={note} onChange={event => setNote(event.target.value)} placeholder="اكتب الملاحظة؛ وهي إلزامية عند إعادة التذكرة" className="mt-4 min-h-24 w-full rounded-xl border border-emerald-200 bg-white p-3" /><div className="mt-3 flex flex-wrap gap-2"><button disabled={review.isPending} onClick={() => review.mutate({ itemId: item.id, approved: true, note }, { onSuccess: () => setMessage('تم اعتماد الموقع بنجاح.') })} className="rounded-xl bg-emerald-700 px-5 py-2.5 font-bold text-white disabled:opacity-50">اعتماد الموقع</button><button disabled={!note.trim() || review.isPending} onClick={() => review.mutate({ itemId: item.id, approved: false, note }, { onSuccess: () => setMessage('أُعيد الموقع إلى المسؤول مع الملاحظة.') })} className="rounded-xl bg-amber-600 px-5 py-2.5 font-bold text-white disabled:opacity-50">إعادة إلى المسؤول</button></div></article>}
      </main>
