...
export function VehicleCard({vehicle,action}:{vehicle:GarageVehicle;action?:React.ReactNode}){
  ...
  return <article className="group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-cyan-300 hover:shadow-xl" data-testid={`vehicle-card-${vehicle.id}`}>
    <Link to={`/central-garage/vehicles-database/${vehicle.id}`} className="block">
      ...
      <div className="p-4"><div className="mb-2 flex flex-wrap gap-1.5"><span className="rounded-full bg-cyan-50 px-2 py-1 text-[10px] font-black text-cyan-800">{GARAGE_VEHICLE_CATEGORY_LABELS[vehicle.vehicleCategory]}</span><span className={`rounded-full px-2 py-1 text-[10px] font-black ${vehicle.ownershipType==='rented'?'bg-violet-50 text-violet-800':'bg-emerald-50 text-emerald-800'}`}>{GARAGE_OWNERSHIP_LABELS[vehicle.ownershipType]}</span></div><div className="flex items-start justify-between gap-3"><h2 className="font-black text-slate-900">{vehicle.vehicleName}</h2><span className={`inline-flex shrink-0 items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-bold ${shift.color}`}><ShiftIcon size={13}/>{shift.label}</span></div>
