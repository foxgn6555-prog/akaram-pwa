...
export default function CentralGarageDashboardPage() {
  ...
  const totalConsumption = useMemo(() => Object.values(data?.consumptionByUnit ?? {}).reduce((sum, item) => sum + Number(item ?? 0), 0), [data])
  const lowStock = data?.tankStock.filter((tank) => tank.lowStock).length ?? 0
  const selectedUnits = [...new Set(data?.tankStock.map((tank) => tank.unit) ?? [])]
  const comparableConsumption = Boolean(filter.fuelType && selectedUnits.length === 1)
  ...
  return <section className="space-y-5" dir="rtl" data-testid="central-garage-dashboard">
    ...
    {summary.isLoading ? <div className="rounded-3xl border bg-white p-12 text-center font-bold text-slate-500">جارٍ تجهيز مؤشرات الكراج…</div> : summary.isError || !data ? <div role="alert" className="rounded-3xl border border-red-200 bg-red-50 p-6 text-center font-bold text-red-700">تعذر تحميل لوحة القيادة. تحقق من الفترة وأعد المحاولة.</div> : <>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <StatCard title="الانطلاقات الفعلية" value={data.dispatchesTotal} hint="ضمن الفترة المحددة" icon={MapPin} tone="bg-violet-50 text-violet-800" />
        <StatCard title={comparableConsumption?'استهلاك الفترة':'الخزانات الفعالة'} value={comparableConsumption?totalConsumption:data.tankStock.length} hint={comparableConsumption?garageUnitLabel(selectedUnits[0]):'اختر مادة ذات وحدة واحدة لقياس الاستهلاك'} icon={Fuel} tone="bg-amber-50 text-amber-800" />
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        <article className="rounded-3xl border bg-white p-5 shadow-sm xl:col-span-2"><div className="flex items-center justify-between"><div><h2 className="flex items-center gap-2 font-black text-slate-900"><BarChart3 size={19} className="text-cyan-700" />الاستهلاك اليومي</h2><p className="mt-1 text-xs text-slate-400">من {data.from} إلى {data.to}</p></div></div>{!comparableConsumption&&<p className="mt-5 rounded-2xl bg-amber-50 p-4 text-xs font-bold text-amber-800">اختر مادة واحدة لعرض الرسم دون جمع وحدات مختلفة.</p>}{comparableConsumption&&<div className="mt-5 flex h-52 min-w-full items-end gap-1 overflow-x-auto border-b border-slate-100 pb-1">{data.dailyConsumption.map((item) => <div key={item.date} title={`${item.date}: ${number(item.quantity)}`} className="group flex h-full min-w-4 flex-1 items-end"><div className="w-full rounded-t bg-cyan-500 transition hover:bg-cyan-700" style={{ height: `${Math.max(item.quantity ? 4 : 1, item.quantity / maxDaily * 100)}%` }} /></div>)}</div>}<div className="mt-2 flex justify-between text-[10px] text-slate-400"><span>{data.from}</span><span>{data.to}</span></div><div className="mt-4 border-t pt-3"><p className="mb-2 text-xs font-black text-slate-600">الاستهلاك الشهري</p><div className="flex flex-wrap gap-2">{data.monthlyConsumption.length ? data.monthlyConsumption.map((item) => <span key={item.month} className="rounded-xl bg-slate-100 px-3 py-2 text-xs"><b>{item.month}</b> · {number(item.quantity)} {comparableConsumption?garageUnitLabel(selectedUnits[0]):''}</span>) : <span className="text-xs text-slate-400">لا توجد بيانات شهرية</span>}</div></div></article>
