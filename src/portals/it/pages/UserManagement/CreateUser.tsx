...
export default function CreateUser() {
  ...
  const toggleSectorGroup = (ids: number[]): void => {
    ...
    updateManagerAreas(
      allSelected ? mgrSectors.filter((id) => !ids.includes(id)) : [...mgrSectors, ...ids],
    )
  }
  ...
  const sectorGroups = [
    {
      key: 'karrada',
      label: 'قاطع الكرادة',
      areas: (sectors ?? []).filter(
        (sector) => sector.parent_sector === 'karrada' || (!sector.parent_sector && sector.id <= 4),
      ),
    },
    {
      key: 'zaafaraniya',
      ...
      areas: (sectors ?? []).filter(
        (sector) =>
          sector.parent_sector === 'zaafaraniya' || (!sector.parent_sector && sector.id >= 5),
      ),
    },
  ]
  ...
  return (
    <section aria-labelledby="create-user-title" className="mx-auto max-w-2xl space-y-4">
      <div>
        <h1 id="create-user-title" className="text-lg font-bold">
          إنشاء مستخدم جديد
        </h1>
      ...
      >
        ...
        >
          <div className="relative">
            <input
              ...
              className={clsx(inputClass(!!errors.password), 'pe-11')}
              {...register('password')}
            />
            <button
              type="button"
        ...
        {isGarageOfficer && (
          <fieldset
            data-testid="garage-assignment"
            className="rounded-xl border border-cyan-200 bg-cyan-50/40 p-4"
          >
            <legend className="px-1 text-xs font-semibold text-cyan-800">
              ربط حساب الكراج بقاطع (مطلوب)
            </legend>
            <Field
              label="قاطع الكراج"
              htmlFor="cu-garage-sector"
              error={errors.garage_parent_sector?.message}
              hint="سيشاهد الحساب آليات مناطق هذا القاطع فقط، دون صلاحية تعديل بياناتها."
            >
              <select
                id="cu-garage-sector"
                data-testid="garage-parent-sector"
                className={inputClass(!!errors.garage_parent_sector)}
                {...register('garage_parent_sector')}
              >
        ...
        {isManager && (
          <fieldset
            data-testid="manager-assignment"
            className="rounded-xl border border-brand-200 bg-brand-50/40 p-4"
          >
            <legend className="px-1 text-xs font-semibold text-brand-700">
              إسناد مسؤول القسم (مطلوب)
            </legend>
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="الشفت" htmlFor="cu-mshift" error={errors.manager_shift?.message}>
                <select
                  id="cu-mshift"
                  data-testid="manager-shift"
                  value={mgrShift}
                  onChange={(e) => {
            ...
            <div className="mt-4" data-testid="manager-sectors">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                <span className="text-sm font-medium">
                  ...
                  <span className="ms-1 text-xs font-normal text-slate-500">
                    (اخترت {mgrSectors.length} من 8 مناطق)
                  </span>
                </span>
                <button
                  type="button"
                  onClick={() => toggleSectorGroup((sectors ?? []).map((sector) => sector.id))}
                  className="rounded-lg border border-brand-200 bg-white px-3 py-1.5 text-xs font-bold text-brand-700 hover:bg-brand-50"
                >
                  {mgrSectors.length === (sectors ?? []).length && mgrSectors.length > 0
                    ? 'إلغاء اختيار الجميع'
                    : 'اختيار جميع القواطع والمناطق'}
                </button>
              </div>
                      ...
                      <div className="mb-3 flex items-center justify-between gap-2">
                        <div>
                          ...
                          <p className="text-[11px] text-slate-500">
                            {ids.filter((id) => mgrSectors.includes(id)).length} من {ids.length}{' '}
                            مناطق
                          </p>
                        </div>
                        <button
                          type="button"
                          onClick={() => toggleSectorGroup(ids)}
                          ...
                          return (
                            <label
                              ...
                              className={clsx(
                                ...
                                on
                                  ? 'border-brand-500 bg-brand-600 font-bold text-white'
              ...
              <p className="mt-2 text-[11px] text-slate-500">
                يمكن إسناد منطقة واحدة أو قاطع كامل أو القاطعين معاً. يرى المسؤول المناطق المسندة
                إليه فقط، ويستمر عزل البيانات على مستوى قاعدة البيانات.
              </p>
            </div>
          </fieldset>
        )}
