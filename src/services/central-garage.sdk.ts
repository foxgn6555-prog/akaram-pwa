...
function vehicleRow(row: Record<string, unknown>): GarageVehicle {
  return {
    ...
    vehicleCategory: (row.vehicle_category as GarageVehicle['vehicleCategory']) ?? 'other',
    ownershipType: (row.ownership_type as GarageVehicle['ownershipType']) ?? 'owned',
    lessorName: (row.lessor_name as string | null) ?? null,
    rentalContractNo: (row.rental_contract_no as string | null) ?? null,
    rentalStartDate: (row.rental_start_date as string | null) ?? null,
    rentalEndDate: (row.rental_end_date as string | null) ?? null,
    modelYear: row.model_year == null ? null : Number(row.model_year),
    vehicleColor: (row.vehicle_color as string | null) ?? null,
...
function tankRow(row: Record<string, unknown>): GarageTank {
  return {
    id: String(row.id),
    fuelType: row.fuel_type as GarageFuelType,
    tankName: String(row.tank_name),
    unit: (row.unit as GarageFuelUnit) ?? 'liter',
...
function movementRow(row: Record<string, unknown>): GarageInventoryMovement {
  const tank = (Array.isArray(row.garage_tanks) ? row.garage_tanks[0] : row.garage_tanks) as
    Record<string, unknown> | null | undefined
  return {
    id: String(row.id),
    tankId: String(row.tank_id),
    vehicleId: (row.vehicle_id as string | null) ?? null,
    movementType: row.movement_type as GarageInventoryMovement['movementType'],
...
function zeroRequestRow(row: Record<string, unknown>): GarageTankZeroRequest {
  return {
    ...
    status: row.status as GarageTankZeroRequest['status'],
...
function departureRow(row: Record<string, unknown>): GarageDeparture {
  return {
    ...
    siteDepartedAt: (row.site_departed_at as string | null) ?? null,
    returnedAt: (row.returned_at as string | null) ?? null,
    recipientManagerId: (row.recipient_manager_id as string | null) ?? null,
    recipientManagerName: (row.recipient_manager_name as string | null) ?? null,
    arrivalNotes: (row.arrival_notes as string | null) ?? null,
    siteDepartureNotes: (row.site_departure_notes as string | null) ?? null,
    notes: (row.notes as string | null) ?? null,
    vehicleName: String(row.vehicle_name),
    dbNumber: String(row.db_number),
    imagePath: String(row.image_path),
...
async function uploadVehicleImage(file: File): Promise<string> {
  if (!imageTypes.has(file.type) || file.size < 1 || file.size > 15 * 1024 * 1024) {
    throw new SDKError(
      'صورة الآلية يجب أن تكون JPG أو PNG أو WebP وبحجم لا يتجاوز 15MB',
      'GARAGE_IMAGE_INVALID',
    )
  }
  ...
  if (authError || !auth.user)
    throw new SDKError('جلسة المستخدم غير متاحة', 'GARAGE_UNAUTHENTICATED', authError)
  const extension = file.type === 'image/png' ? 'png' : file.type === 'image/webp' ? 'webp' : 'jpg'
  const path = `${auth.user.id}/${crypto.randomUUID()}.${extension}`
  await sdkGuard(
    supabase.storage.from('garage-vehicles').upload(path, file, { contentType: file.type }),
  )
  return path
}
...
export const centralGarage = {
  ...
  async vehicles(filter: GarageVehicleFilter = {}): Promise<GarageVehiclePage> {
    ...
    const signed = rows.length
      ? await sdkGuard(
          supabase.storage.from('garage-vehicles').createSignedUrls(
  ...
  async vehicle(vehicleId: string): Promise<GarageVehicle> {
    ...
    const vehicle = vehicleRow({
      ...
      parent_sector: sector.parent_sector,
    })
    const signed = await sdkGuard(
      supabase.storage.from('garage-vehicles').createSignedUrl(vehicle.imagePath, 600),
    )
  ...
  async createVehicle(input: CreateGarageVehicleInput): Promise<GarageVehicle> {
    ...
    try {
      const data = await sdkGuard(
        supabase.rpc('garage_add_vehicle', {
          p_vehicle_name: input.vehicleName,
          p_db_number: input.dbNumber,
          ...
          p_lessor_name: input.lessorName || null,
          p_rental_contract_no: input.rentalContractNo || null,
          p_rental_start_date: input.rentalStartDate || null,
          p_rental_end_date: input.rentalEndDate || null,
          p_model_year: input.modelYear ?? null,
          p_vehicle_color: input.vehicleColor || null,
          p_specifications: input.specifications || null,
        }),
      )
  ...
  async updateVehicle(
    ...
    try {
      const data = await sdkGuard(
        supabase.rpc('garage_update_vehicle', {
          p_vehicle_id: id,
          p_vehicle_name: input.vehicleName,
          p_db_number: input.dbNumber,
          p_plate_number: input.plateNumber,
  ...
  async assignDriver(
    ...
    sectorId: number,
    reason?: string,
  ): Promise<GarageDriverAssignment> {
    const data = await sdkGuard(
  ...
  async assignmentHistory(vehicleId: string): Promise<GarageDriverAssignment[]> {
    const rows = await sdkGuard(
      supabase
        .from('garage_driver_assignments')
        .select('*')
  ...
  async archiveVehicle(vehicleId: string, reason: string): Promise<void> {
    await sdkVoid(
      supabase.rpc('garage_archive_vehicle', { p_vehicle_id: vehicleId, p_reason: reason.trim() }),
    )
  },

  async restoreVehicle(vehicleId: string, reason: string): Promise<GarageVehicle> {
    const data = await sdkGuard(
      supabase.rpc('garage_restore_vehicle', { p_vehicle_id: vehicleId, p_reason: reason.trim() }),
    )
    return vehicleRow(data as unknown as Record<string, unknown>)
  },
  ...
  async addTank(
    ...
    const data = await sdkGuard(
      supabase.rpc('garage_add_tank', {
        p_fuel_type: fuelType,
        p_tank_name: tankName,
        p_unit: unit,
        p_capacity: capacity,
  ...
  async addTankStock(
    ...
    quantity: number,
    notes?: string,
  ): Promise<GarageInventoryMovement> {
    const data = await sdkGuard(
      supabase.rpc('garage_add_tank_stock', {
        p_tank_id: tankId,
        p_quantity: quantity,
  ...
  async fillVehicle(
    ...
    const data = await sdkGuard(
      supabase.rpc('garage_fill_vehicle', {
        p_tank_id: tankId,
        p_vehicle_id: vehicleId,
        p_quantity: quantity,
        p_next_refill_date: nextRefillDate ?? null,
        p_notes: notes?.trim() || null,
      }),
    )
    return movementRow(data as unknown as Record<string, unknown>)
  ...
  async movements(
    filter: { tankId?: string; vehicleId?: string } = {},
  ): Promise<GarageInventoryMovement[]> {
    let query = supabase
      .from('garage_inventory_movements')
      .select('*,garage_tanks!inner(fuel_type,unit,tank_name)')
  ...
  async requestTankZero(tankId: string, reason: string): Promise<GarageTankZeroRequest> {
    const data = await sdkGuard(
      supabase.rpc('garage_request_tank_zero', { p_tank_id: tankId, p_reason: reason.trim() }),
    )
  ...
  async zeroRequests(status?: GarageTankZeroRequest['status']): Promise<GarageTankZeroRequest[]> {
    let query = supabase
      .from('garage_tank_zero_requests')
      .select('*')
  ...
  async decideTankZero(
    requestId: string,
    approved: boolean,
    note?: string,
  ): Promise<GarageTankZeroRequest> {
    const data = await sdkGuard(
      supabase.rpc('garage_decide_tank_zero', {
  ...
  async todayDepartures(): Promise<GarageDeparture[]> {
    ...
    const signed = departures.length
      ? await sdkGuard(
          supabase.storage.from('garage-vehicles').createSignedUrls(
            departures.map((row) => row.imagePath),
            600,
          ),
        )
      : []
  ...
  async departureDays(limit = 60, offset = 0): Promise<GarageTripDay[]> {
    const rows = await sdkGuard(
      supabase.rpc('garage_departure_days', { p_limit: limit, p_offset: offset }),
    )
    return ((rows ?? []) as unknown as Record<string, unknown>[]).map((r) => ({
      tripDay: String(r.trip_day),
      totalCount: Number(r.total_count),
      openCount: Number(r.open_count),
      firstDepartureAt: String(r.first_departure_at),
      lastActivityAt: String(r.last_activity_at),
    }))
  },

  async departuresForDay(day: string): Promise<GarageDeparture[]> {
    const data = await sdkGuard(supabase.rpc('garage_departures_for_day', { p_day: day }))
    const departures = ((data ?? []) as unknown as Record<string, unknown>[]).map(departureRow)
    const signed = departures.length
      ? await sdkGuard(
          supabase.storage.from('garage-vehicles').createSignedUrls(
            departures.map((r) => r.imagePath),
            600,
          ),
        )
      : []
    departures.forEach((r, i) => {
      if (signed[i]?.signedUrl) r.imageUrl = signed[i].signedUrl
    })
    return departures
  },

  async shiftAssignments(vehicleId: string): Promise<GarageVehicleShiftAssignment[]> {
    const rows = await sdkGuard(
      supabase.rpc('garage_vehicle_shift_assignments_list', { p_vehicle_id: vehicleId }),
    )
    return ((rows ?? []) as unknown as Record<string, unknown>[]).map((r) => ({
      id: String(r.id),
  ...
  async shiftDispatchRecipients(
    vehicleId: string,
    shift: GarageShift,
  ): Promise<GarageDispatchRecipient[]> {
    const rows = await sdkGuard(
      supabase.rpc('garage_shift_dispatch_recipients', { p_vehicle_id: vehicleId, p_shift: shift }),
    )
    return ((rows ?? []) as unknown as Record<string, unknown>[]).map((r) => ({
  ...
  async dispatchRecipients(vehicleId: string): Promise<GarageDispatchRecipient[]> {
    const rows = await sdkGuard(
      supabase.rpc('garage_dispatch_recipients', { p_vehicle_id: vehicleId }),
    )
    return ((rows ?? []) as unknown as Record<string, unknown>[]).map((r) => ({
      userId: String(r.user_id),
      managerName: String(r.manager_name),
  ...
  async recordDeparture(vehicleId: string, notes?: string): Promise<GarageDeparture> {
    const data = await sdkGuard(
      supabase.rpc('garage_record_departure', {
        p_vehicle_id: vehicleId,
        p_notes: notes?.trim() || null,
      }),
    )
  ...
  async recordReturn(departureId: string): Promise<GarageDeparture> {
    const data = await sdkGuard(
      supabase.rpc('garage_record_return', { p_departure_id: departureId }),
    )
    return departureRow(data as unknown as Record<string, unknown>)
  },
  ...
  async dashboard(filter: GarageDashboardFilter = {}): Promise<GarageDashboardSummary> {
    const data = await sdkGuard(
      supabase.rpc('garage_dashboard_summary', {
        p_from: filter.from ?? null,
        p_to: filter.to ?? null,
        p_sector_id: filter.sectorId ?? null,
        p_fuel_type: filter.fuelType ?? null,
      }),
    )
  ...
  async report(filter: GarageReportFilter = {}): Promise<GarageReportResult> {
    ...
    const page = Math.max(1, Math.trunc(filter.page ?? 1))
    const data = await sdkGuard(
      supabase.rpc('garage_consumption_report', {
        p_from: filter.from ?? null,
        p_to: filter.to ?? null,
        p_sector_id: filter.sectorId ?? null,
        p_fuel_type: filter.fuelType ?? null,
        p_tank_id: filter.tankId ?? null,
        p_vehicle_id: filter.vehicleId ?? null,
        p_vehicle_ids: filter.vehicleIds ?? null,
  ...
  async reportAll(
    ...
    for (let page = 2; page <= pages; page += 1) {
      const next = await centralGarage.report({ ...filter, page, pageSize: 100 })
      rows.push(...next.rows)
    }
    return { ...first, rows }
  },
