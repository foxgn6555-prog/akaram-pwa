-- 00062 · تدقيق تذكرة البريد + المسؤول كوحدة ذرية بدلاً من تدقيق كل صورة منفردة.
...
begin
 ...
 if p_approved then
  update public.complaints set status=case when not exists(select 1 from public.complaint_items where complaint_id=p_complaint_id and status<>'approved')then'ready_to_send'else'quality_review'end where id=p_complaint_id;
 else
  update public.complaints set status='in_progress' where id=p_complaint_id;
  insert into public.notifications(user_id,title,body,type,link)values(p_manager_id,'أُعيدت تذكرة معالجة كاملة',coalesce(v_reference,'')||' — '||trim(p_note),'warning','/manager/complaints/'||p_complaint_id::text);
 end if;
 return v_count;
end $$;
