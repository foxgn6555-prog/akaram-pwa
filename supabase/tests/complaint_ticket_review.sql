...
do $$declare officer uuid:='10000000-0000-0000-0000-000000000071';manager uuid:='10000000-0000-0000-0000-000000000072';msg uuid;cid uuid;reviewed integer;begin
 ...
 insert into public.complaint_media(item_id,media_kind,storage_path,original_name,mime_type,size_bytes,sha256,source,uploaded_by,media_code,is_active)
 select id,'after','item/'||id::text||'/after/review.jpg','review.jpg','image/jpeg',3,repeat('a',64),'gallery',manager,'REV-'||replace(id::text,'-',''),true from public.complaint_items where complaint_id=cid;
